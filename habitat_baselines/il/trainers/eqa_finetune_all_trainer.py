...
from habitat_baselines.il.models.models import NavPlannerControllerModel
from habitat_baselines.utils.common import generate_video

cv2 = try_cv2_import()


@baseline_registry.register_trainer(name="eqa_finetune_all")
class EQAFinetuneAllTrainer(BaseILTrainer):
    r"""Trainer class for finetuning all modules together for
     EmbodiedQA (Das et. al.;CVPR 2018)
    ...
    # TODO: insert question and answers into the video
    ...
    def _eval_checkpoint(
        ...
        with habitat.Env(config.TASK_CONFIG) as env:
            nav_dataset = NavDataset(config, env, self.device).decode("rgb")
