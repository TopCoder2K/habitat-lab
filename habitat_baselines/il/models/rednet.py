...
class RedNetResizeWrapper(nn.Module):
    ...
    def forward(self, rgb, depth):
        r"""
            Args:
                Raw sensor inputs.
                rgb: B x 3 x H=256 x W=256
                depth: B x 1 x H x W
            Returns:
                semantic: drop-in replacement for default semantic sensor.
                B x 1 x H x W  (no channel, for some reason)
        """
        # Not quite sure what depth is produced here. Let's just check
        if self.resize:
            _, _, og_h, og_w, = rgb.size()  # b c h w
