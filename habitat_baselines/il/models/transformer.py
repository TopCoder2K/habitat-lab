...
class Transformer(nn.Module):
    def __init__(
        ...
        encoder_layer = TransformerEncoderLayer(d_model, nheads, dim_feedforward, dropout, activation, normalize_before)
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            if isinstance(text[0], str):
                ...
                # our case: last_hidden_state, pooler_output
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nheads, dim_feedforward=2048, dropout=0.1, activation="relu", normalize_before=False):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(d_model, nheads, dropout=dropout)
        self.cross_attn_image = nn.MultiheadAttention(d_model, nheads, dropout=dropout)
        # self.cross_attn_text = nn.MultiheadAttention(d_model, nheads, dropout=dropout)
...
class FeatureResizer(nn.Module):
    """
    This class takes as input a set of embeddings of dimension C1 and
    outputs a set of embeddings of dimension C2, after a linear transformation,
    dropout and normalization (LN).
    """
...
def build_transformer(args):
    ...
    return Transformer(
        d_model=args.IL.TRANSFORMER.d_model,
        nheads=args.IL.TRANSFORMER.nheads,
        num_encoder_layers=args.IL.TRANSFORMER.enc_layers,
        num_decoder_layers=args.IL.TRANSFORMER.dec_layers,
        ...
        freeze_text_encoder=args.IL.TRANSFORMER.freeze_text_encoder,
