...
from .misc import NestedTensor, interpolate
...
class MDETR(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            ans_vocab_size: number of words in the vocabulary for the answers.
            ...
            if split_qa_heads:
                ...
                if qa_dataset == "gqa":
                    ...
                    self.answer_head = nn.Linear(
                        hidden_dim,
                        1944 if ans_vocab_size is None else ans_vocab_size
                    )

    def forward(self, samples: torch.Tensor, captions, encode_and_save=True,
                memory_cache=None):
        ...
        """
        # TODO: check the assumption that the last frame is sufficient
        samples = samples[:, 0, :, :, :]
...
class QACriterionVQA(nn.Module):
    ...
    def forward(self, output, answers):
        ...
        # TODO: it's expected that loss is not dict
        device = output["pred_answer_type"].device
        loss["loss_answer_type"] = F.cross_entropy(output["pred_answer_type"], answers["answer_type"])
...
def build_mdetr(device, args, ans_vocab_size=None):
    ...
    model = MDETR(
        ...
        num_queries=args.IL.MDETR.num_queries,
        ans_vocab_size=ans_vocab_size,
        aux_loss=args.IL.MDETR.aux_loss,
        contrastive_hdim=args.IL.MDETR.contrastive_loss_hdim,
        contrastive_loss=args.IL.MDETR.contrastive_loss,
        contrastive_align_loss=args.IL.MDETR.contrastive_align_loss,
    ...
    # criterion = None
    # if not args.no_detection.IL.MDETR:
